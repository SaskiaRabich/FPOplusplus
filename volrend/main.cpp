...
#include "volrend/n3fouriertree.hpp"
...
void glfw_update_title(GLFWwindow* window) {
    ...
    if (elapsed > 0.5) {
        ...
        char tmp[128];
        sprintf(tmp, "FourierPlenOctree viewer - FPS: %.2f", fps);
...
void draw_imgui(VolumeRenderer& rend, N3FourierTree& tree) {
    ...
    if (open_tree_dialog.GetTitle().empty()) {
        ...
        open_tree_dialog.SetTitle("Load N3FourierTree npz from svox");
    }
    ...
    if (open_tree_dialog.HasSelected()) {
        ...
        printf("Load N3FourierTree npz: %s\n", path.c_str());
        tree.open(path);
        rend.set(tree);
    ...
    if (ImGui::CollapsingHeader("Dynamic")) {
        static int render_fixed_timestep = 0;
        static bool curr_forward = true;
        static float ts_tmp = rend.options.timestep;
        static bool fix_fps = true;
        static int update_step = 1;
        ImGui::TextUnformatted("Movement: ");
        ImGui::SameLine();
        ImGui::RadioButton("dynamic", &render_fixed_timestep,
                           0);
        ImGui::SameLine();
        ImGui::RadioButton("static", &render_fixed_timestep, 1);
        if ((render_fixed_timestep == 0) && (tree.timesteps > 1)) {
            ImGui::Checkbox("Fix time steps per second", &fix_fps);
            //ImGui::SameLine();
            if (fix_fps) {
                ImGui::TextUnformatted("Time steps per second: ");
                ImGui::InputFloat(" ", &rend.options.fps);
                ImGui::Checkbox("Render backwards movement", &rend.options.render_backwards);
                double time = glfwGetTime();
                if ((time - rend.options.stamp_previous)> 1./rend.options.fps) {
                    update_step = (int ((time- rend.options.stamp_previous)*1000))/(int ((1./rend.options.fps)*1000));
                    if (rend.options.render_backwards) {
                        rend.options.stamp_previous = time;
                        if (curr_forward) {
                            rend.options.timestep += (float)update_step;
                            if (rend.options.timestep > (float)tree.timesteps-1.f) {
                                rend.options.timestep = (float)tree.timesteps-2.f;
                                curr_forward = false;
                            }
                        } else {
                            rend.options.timestep -= (float)update_step;
                            if (rend.options.timestep < 0.f) {
                                rend.options.timestep = 1.f;
                                curr_forward = true;
                            }
                        }
                    } else {
                        ...
                        rend.options.timestep += (float)update_step;
                        if (rend.options.timestep > (float)tree.timesteps-1.f) {
                            rend.options.timestep = 0.f;
                        }
                    }
                }

            } else {
                ImGui::InputFloat("Speed", &rend.options.dyn_speed);
                if (rend.options.dyn_speed > tree.timesteps/2.f) {
                    rend.options.dyn_speed = tree.timesteps/2.f;
                }
                if (rend.options.dyn_speed < 0.f) {
                    rend.options.dyn_speed = 0.001f;
                }
                ImGui::Checkbox("Render backwards movement", &rend.options.render_backwards);
                if (rend.options.render_backwards && curr_forward) {
                    rend.options.timestep += rend.options.dyn_speed;
                    if (rend.options.timestep > (float)tree.timesteps-1.f) {
                        rend.options.timestep -= 2* rend.options.dyn_speed;
                        curr_forward = false;
                    }
                } else if (rend.options.render_backwards && !curr_forward){
                    rend.options.timestep -= rend.options.dyn_speed;
                    if (rend.options.timestep < 0.f) {
                        rend.options.timestep += 2* rend.options.dyn_speed;
                        curr_forward = true;
            ...
            ImGui::SliderFloat("Time step", &rend.options.timestep, 0.f,
                           (float)tree.timesteps-1.f);
            ImGui::TextUnformatted("Set to time step: ");
            ImGui::InputFloat(" ", &ts_tmp);
            if (ImGui::Button("Update")) {
                rend.options.timestep = ts_tmp;
            }
        }
        ImGui::Checkbox("Round timestep", &rend.options.round_timestep);
