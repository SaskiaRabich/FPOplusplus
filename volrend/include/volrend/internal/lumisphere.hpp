...
VOLREND_COMMON_FUNCTION float IDFT(
    const int i, const float t, const int T) {
    float ret;
    if (i%2 == 0) {
...
VOLREND_COMMON_FUNCTION void precalc_fourier_basis(
    const int dim , const float t, const int T,
...
VOLREND_COMMON_FUNCTION void maybe_precalc_basis(
    const FTreeSpec& VOLREND_RESTRICT tree, const scalar_t* VOLREND_RESTRICT dir,
    scalar_t* VOLREND_RESTRICT out) {
    ...
    switch (tree.data_format.format) {
        ...
        }  // SH / Anything using SH coefficients
        break;
