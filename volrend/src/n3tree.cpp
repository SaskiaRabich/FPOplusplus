...
void DataFormat::parse(const std::string& str) {
    ...
    if (~nonalph_idx) {
        ...
        if (tmp == "FC" || tmp == "LFC") {
            ...
            while(std::getline(str_stream, tmp_str, '_'))
            {
                str_list.push_back(tmp_str);
            }
            fc_dim_sigma = std::atoi(str_list[0].c_str() + nonalph_idx);
            fc_dim_rgb = std::atoi(str_list[1].c_str());
            basis_dim = std::atoi(str_list[2].c_str());
        } else {
...
std::string DataFormat::to_string() const {
    ...
    if (~fc_dim_sigma) {
        out.append(std::to_string(fc_dim_sigma));
        out.append("_");
    }
    if (~fc_dim_rgb) {
        out.append(std::to_string(fc_dim_rgb));
        out.append("_");
    }
