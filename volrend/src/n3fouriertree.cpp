...
N3FourierTree::N3FourierTree() {}
N3FourierTree::N3FourierTree(const std::string& path) { open(path); }
N3FourierTree::~N3FourierTree() {
#ifdef VOLREND_CUDA
    free_cuda();
#endif
...
void N3FourierTree::open(const std::string& path) {
    clear_cpu_memory();
...
void N3FourierTree::open_mem(const char* data, uint64_t size) {
    data_loaded_ = false;
#ifdef VOLREND_CUDA
    cuda_loaded_ = false;
#endif
...
// namespace {
// int _calc_tree_maxdepth(const N3FourierTree& tree, size_t nodeid, size_t xi, size_t
// yi,
//                         size_t zi) {
//     const int32_t* child =
...
// void _calc_occu_lut(N3FourierTree& tree, size_t nodeid, uint32_t xi, uint32_t yi,
...
void N3FourierTree::load_npz(cnpy::npz_t& npz) {
    ...
    if (npz.count("time_steps")){
        timesteps = (int)*npz["time_steps"].data<int64_t>();
    }
    else {
        timesteps = 1;
    }

    if (npz.count("augmented_time")){
        augmented_time = (bool)*npz["augmented_time"].data<bool>();
    }
    else {
...
void _gen_wireframe_impl(const N3FourierTree& tree, size_t nodeid, size_t xi,
...
std::vector<float> N3FourierTree::gen_wireframe(int max_depth) const {
...
bool N3FourierTree::is_data_loaded() { return data_loaded_; }
#ifdef VOLREND_CUDA
bool N3FourierTree::is_cuda_loaded() { return cuda_loaded_; }
#endif

void N3FourierTree::clear_cpu_memory() {
...
int N3FourierTree::pack_index(int nd, int i, int j, int k) {
    assert(i < N && j < N && k < N && i >= 0 && j >= 0 && k >= 0);
    return nd * N3_ + i * N2_ + j * N + k;
}

std::tuple<int, int, int, int> N3FourierTree::unpack_index(int packed) {
    int k = packed % N;
    packed /= N;
    int j = packed % N;
    packed /= N;
