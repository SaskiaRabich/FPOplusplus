...
__host__ __device__ __inline__ void _point2refBox(
        ...
        const float *const __restrict__ extra_data,
        const float t,
        const int T) {
    int t0 = (int)t;
    int t1 = min((int)t+1,T-1);
    float weight = t - (float)t0;
    for(int i=0;i<3;++i){
        float off = extra_data[i] - (extra_data[3*t0+i] * (1.f-weight) + extra_data[3*t1+i] * weight);
        p[i]=p[i] + off;
    }
    return;
}
...
render_kernel(
    ...
    if (opt.enable_probe && y < opt.probe_disp_size + 5 &&
        ...
        float fc_basis[VOLREND_GLOBAL_FC_MAX];
        ...
        if (c <= 1.f) {
            ...
            if (tree.data_format.basis_dim >= 0) {
                ...
                if (tree.data_format.format == DataFormat::FC 
                    ...
                    float ts = opt.timestep;
                    if (opt.round_timestep) {
                        ts = floor(ts+0.5);
                    }
                    if (tree.augmented_time) {
                        ts += 1.f;
                        internal::precalc_fourier_basis(tree.data_format.fc_dim_rgb, ts, tree.timesteps+2, fc_basis);
                    } else {
                        internal::precalc_fourier_basis(tree.data_format.fc_dim_rgb, ts, tree.timesteps, fc_basis);
                    }
                    // fourier to sh calculation
                    float sh_tmp[3*VOLREND_GLOBAL_BASIS_MAX];
                    for (int t = 0; t < 3; ++t){
                        int off = t * tree.data_format.basis_dim * tree.data_format.fc_dim_rgb;
                        for (int j = 0; j < tree.data_format.basis_dim; ++j){
                            float tmp = 0.0;
                            int off2 = off + j*tree.data_format.fc_dim_rgb;
                            for (int i = 0; i < tree.data_format.fc_dim_rgb; ++i){
                                tmp += fc_basis[i] * probe_coeffs[off2 +i];
                            }
    ...
    if (enable_draw) {
        ...
        if ((tree.data_format.format == DataFormat::FC || tree.data_format.format == DataFormat::LFC) && tree.extra != nullptr){
            float ts = opt.timestep;
            if (opt.round_timestep) {
                ts = floor(ts+0.5);
            }
            if (tree.augmented_time){
                if (fabs(ts+1.f) < 0.0001f) {
                    ts = 0.;
                } else if (fabs(ts- (float)tree.timesteps) < 0.0001f) {
                    ts = tree.timesteps-1.;
                }
            }
            _point2refBox(cen,tree.extra,ts,tree.timesteps);
        }
...
__host__ void launch_renderer(const N3FourierTree& tree,
